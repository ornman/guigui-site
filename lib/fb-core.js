...
export const LIMITS = {
  ...
  runlog_lines: 4000, runlog_line: 500,   // 2026-09-16 全量口径:guigui.log 工程日志区
  diag_json: 96 * 1024,
  payload: 256 * 1024,         // Content-Length 早拒(不进 JSON 解析)
};
...
function normalizeDiag(d) {
  ...
  // runlog 显式归一(不走 trimDeep:数组 60 上限会把工程日志砍到 60 行)
  if (d.runlog !== undefined) {
    const r = (typeof d.runlog === 'object' && d.runlog !== null && !Array.isArray(d.runlog))
      ? d.runlog : {};
    diag.runlog = {
      bytes: Number(r.bytes) || 0,
      total_lines: Number(r.total_lines) || 0,
      omitted_lines: Number(r.omitted_lines) || 0,
      lines: Array.isArray(r.lines)
        ? r.lines.slice(0, LIMITS.runlog_lines)
            .map((l) => cut(String(l == null ? '' : l), LIMITS.runlog_line))
        : [],
    };
  }
...
export function render_issue(rec) {
  ...
  if (rec.diag.runlog !== undefined) {
    lines.push(renderRunlog(rec.diag.runlog));
    lines.push('');
  }
...
/* runlog 单列渲染:GitHub issue 正文 64K 硬顶,issue 内只入尾部字符预算;
 * 全量行在 D1 diag_json(/fb/list 可取),issue 注明两层截头。 */
const RUNLOG_ISSUE_BUDGET = 12000;
function renderRunlog(rl) {
  const lines = Array.isArray(rl && rl.lines) ? rl.lines : [];
  let start = 0;
  let used = 0;
  for (let i = lines.length - 1; i >= 0; i--) {
    used += String(lines[i] == null ? '' : lines[i]).length + 1;
    if (used > RUNLOG_ISSUE_BUDGET && i < lines.length - 1) { start = i + 1; break; }
  }
  const shown = lines.slice(start);
  const meta = [];
  if (rl && rl.omitted_lines) meta.push(`客户端截头 ${rl.omitted_lines} 行`);
  if (start) meta.push(`issue 内再截头 ${start} 行(全量在诊断库)`);
  const head = `运行日志 runlog(guigui.log,尾 ${shown.length} 行${meta.length ? ' · ' + meta.join(' · ') : ''})`;
  const body = fenced(shown.map(String).join('\n') || '(空)');
