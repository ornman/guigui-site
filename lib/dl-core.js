...
/* version.json → { ver, path, size_hint };任何失败返回 null(发版间隙/资产缺失不挡下载)。 */
export async function resolveAsset({ request, env }) {
  try {
    ...
    return path
      ? { ver: v.latest, path, size_hint: typeof v.size_mb === 'number' ? v.size_mb : null }
      : null;
  } catch { return null; }
}
...
/* 大小探自**公网自域**的资产响应头(ASSETS 绑定实测 HEAD/Range/GET 全不给长度);
 * HEAD 优先,Range GET 兜底,头读完即 cancel 不拉全量。带自定义 UA 防 Bot Fight Mode。 */
async function assetSize(origin, path, fetchImpl) {
...
export async function handleDlCount({ request, env, store, fetchImpl = fetch }) {
  ...
  let size_mb = null;
  if (asset) {
    const bytes = await assetSize(origin, asset.path, fetchImpl);
    size_mb = bytes ? Math.round(bytes / 1048576) : asset.size_hint;   // 探不到 → version.json 兜底
  }
  const rows = await store.allEvents();
