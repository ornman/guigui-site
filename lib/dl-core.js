...
/* version.json → { ver, path, size_hint, dl_base };任何失败返回 null(发版间隙/资产缺失不挡下载)。 */
export async function resolveAsset({ request, env }) {
  try {
    ...
    return path
      ? { ver: v.latest, path, size_hint: typeof v.size_mb === 'number' ? v.size_mb : null, dl_base: normDlBase(v.dl_base) }
      : null;
  } catch { return null; }
}
