...
/* GET /download/count — 公开计数(下载区灰字用):只回真人数,不带任何明细;
 * 5 分钟边缘缓存扛住页面流量;D1 不可达 503 不缓存,页面端静默藏行。 */
export async function handleDlCount({ store }) {
  const rows = await store.allEvents();
  if (rows === undefined) return json({ ok: false }, 503);
  const { human_total } = aggregate(rows);
  return new Response(JSON.stringify({ ok: true, count: human_total }), {
